#ifndef _aes_h_
#define _aes_h_
#include <stdio.h> // ukljucivanje ulaz izlaz
#include "queue.h" // ukljucivanje UChar
typedef unsigned __int32 UInt32; // Ako unsigned int nije 4 bita
#define NUM_ROW 4// broj vrsta koji sadrzi jedna rec
#define STATE_SIZE 16
#define CRYPT_TEXT ".crypt"
#define DIGIT_NUM 10

// Preracunate vrednosti koje koristi funkcija SubBytes
const UChar S_BOX[16][16] = {
...
// Preracunate vrednosti koje koristi funkcija InvSubBytes
const UChar INVS_BOX[16][16] = {
...
// Koristi se za mnozenje u Galoaovom polju
const UChar G_FIELD_MUL[256][6] = {
...
// koristi se kad se stvaraju novi kljucevi od starih
const UInt32 ROUND_CONSTANT[] = { 0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000,
0x40000000, 0x80000000, 0x1b000000, 0x36000000, 0x6c000000, 0xd8000000,
0xab000000, 0x4d000000, 0x9a000000 };

// pretvara heksadecimalnu cifru u binarnu
// vraca 16 ako nije dobra heksadecimalna cifra, inace vrednost
UChar LetterToHex(UChar letter);

// pretvara 2 bajta slova u 1 bajt UChar
// vraca 0 ako je neuspesna konverzija
// vraca 1 ako je uspesna
int UCharLettersToHex(UChar byte1, UChar byte2, UChar *byteHex);


// pretvara 4 bajta u jedan UInt32
UInt32 UCharToUInt32(const UChar byte[]);

// XOr-uje sa kljucem tekst koji treba sifrovati
void AddRoundKey(UChar state[][4],  UInt32 key[]);

// Menja vrednost bajta sa odgovarajucim bajtom iz S_BOX
void SubBytes(UChar state[][4]);

// Menja vrednost bajta sa odgovarajucim bajtom iz INVS_BOX
void InvSubBytes(UChar state[][4]);

// pomera bajtove duz vrsta
void ShiftRows(UChar state[][4]);

// inverzno pomera bajtove duz vrsta
void InvShiftRows(UChar state[][4]);

// mix-uje kolone
void MixColumns(UChar state[][4]);

// vraca kolone gde su bile
void InvMixColumns(UChar state[][4]);

// 1-bajtna kruzna rotacija ulevo, 4-bajtne reci
void RotWord(UInt32 *wordPt);

// Zamena svakog bajta kljuca runde koristeçi S_BOX
UInt32 SubWord(UInt32 word);

// pravi od kljuca key kljuceve w za svaku rundu
// ima ih  koliko i rundi * 4 + 1 (prva i poslednja vrednost se XOR)
// vraca 0 ako je neuspesna ekspanzija (nedovoljno memorije)
// vraca 1 ako je uspesna ekspanzija
int KeyExpansion(const UChar key[], UInt32 **w, const int keySize);

// Sifruje niz karaktera zadat sa in pomocu niza kljuceva key (dobijeni u ekspanziji)
// Vraca niz karaktera zadat sa out
void EncryptState(UChar in[], UChar out[], UInt32 key[], int keySize);

// Desifruje niz karaktera zadat sa in pomocu niza kljuceva key (dobijeni u ekspanziji)
// Vraca niz karaktera zadat sa out
